...
void Triangle::paint() const {//绘制三角形
	material->glSetMaterial();
...
bool Transform::intersect(const Ray& r, Hit& h, float tmin) {
	Vec3f ro = r.getOrigin();
	Vec3f rd = r.getDirection();
	Matrix invM = matrix;//变换矩阵
	if (invM.Inverse()) {//判断矩阵是否可逆,奇异矩阵不可逆
		//先变到object space，球交点
		invM.Transform(ro);//平移射线源
		invM.TransformDirection(rd);//平移射线方向
		Ray ray_local(ro, rd);
		if (object->intersect(ray_local, h, tmin)) {
			Vec3f normal = h.getNormal();//此时的normal在object space,下面的操作转换为world space
			Matrix normalMatrix = invM;
			normalMatrix.Transpose();//转置
			normalMatrix.TransformDirection(normal);//trans normal to world space
			normal.Normalize();//标准化
...
bool Group::intersect(const Ray& r, Hit& h, float tmin) {
	...
	for (int i = 0; i < num_objects; ++i) {
		if (objects[i]->intersect(r, h, tmin)) {//使用了多态，基类调用子类方法
			flag = true;
		}
	}
	return flag;
...
void Group::paint() const {
	for (int i = 0; i < num_objects; ++i) {
		objects[i]->paint();//此处调用到了多态，基类调用子类的方法
	}
}